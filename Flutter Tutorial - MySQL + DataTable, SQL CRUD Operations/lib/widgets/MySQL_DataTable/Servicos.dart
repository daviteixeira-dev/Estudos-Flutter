...
class Services {
  ...
  static Future<List<Funcionario>> getEmployees() async {
    try {
      ...
      if (response.statusCode == 200) {
        List<Funcionario> list = parsePhotos(response.body);
        return list;
      } else {
        throw List<Funcionario>();
      }
    } catch (e) {
  ...
  static List<Funcionario> parsePhotos(String responseBody) {
  ...
  static Future<String> updateEmployee(
    ...
    try {
      ...
      map["emp_id"] = empId;
      map["primeiro_nome"] = firstName;
      map["ultimo_nome"] = lastName;
      final response = await http.post(ROOT, body: map);
